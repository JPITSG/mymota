...
uint32_t configCrc(const StoredConfig &cfg) {
  StoredConfig tmp = cfg;
  tmp.crc = 0;
...
bool saveConfig(const char *ssid, const char *password, const char *hostname) {
  memset(&config, 0, sizeof(config));
  config.magic = kConfigMagic;
  config.version = kConfigVersion;
  config.size = sizeof(StoredConfig);
...
void appendHeader(String &page, const __FlashStringHelper *title) {
  ...
  page += F("input,button{font:inherit;padding:8px;margin:4px 0;box-sizing:border-box}input[type=text],input[type=password],input[type=file]{width:100%;max-width:460px}");
...
void handleRoot() {
  ...
  page += htmlEscape(config.hostname);
  page += F("'></label></div><button type='submit'>Save Wi-Fi</button></form>");
...
void handleWifiSave() {
  ...
  if (!saveConfig(ssid.c_str(), password_to_save, hostname.c_str())) {
