...
void appendButtonActionExtra(String &page, uint8_t button, const char *name, bool hold) {
  ...
  page += F("</textarea></div><p class='hint action-hint'></p></div>");
}
...
void appendButtonSettings(String &page) {
  ...
  page += F("</div><p class='hint'>MQTT broadcast sends a topic and payload through the configured broker.</p></div></div></div><form id='form-buttons' data-inline='1' method='post' action='/buttons'>");
  ...
  page += F("</form><div class='panel-foot'><button type='submit' form='form-buttons'>Save inputs</button></div></section>");
}
