...
bool relay_pulse_pending[kMaxRelays]{};
uint32_t relay_pulse_due[kMaxRelays]{};
...
void setDefaultRelayEnforcementConfig(StoredConfig &target) {
  memset(target.relay_restore_boot, 1, sizeof(target.relay_restore_boot));
...
void setDefaultRelayPulseConfig(StoredConfig &target) {
  memset(target.relay_pulse_enabled, 0, sizeof(target.relay_pulse_enabled));
  memset(target.relay_pulse_seconds, 0, sizeof(target.relay_pulse_seconds));
}

void setDefaultRelayPulseConfig() {
  setDefaultRelayPulseConfig(config);
}
...
uint32_t lastRelaySnapshotFlashBase() {
  return energyJournalFlashBase() - (static_cast<uint32_t>(kLastRelaySnapshotSectorCount) * kFlashSectorSize);
}

uint8_t lastRelaySnapshotRecordCount() {
  return kFlashSectorSize / sizeof(GracefulRelaySnapshot);
}

uint32_t lastRelaySnapshotSlotAddress(uint8_t slot) {
  return lastRelaySnapshotFlashBase() + (static_cast<uint32_t>(slot) * sizeof(GracefulRelaySnapshot));
}

bool readLastRelaySnapshotSlot(uint8_t slot, GracefulRelaySnapshot &snapshot) {
  if (slot >= lastRelaySnapshotRecordCount()) return false;
  return ESP.flashRead(lastRelaySnapshotSlotAddress(slot),
                       reinterpret_cast<uint32_t *>(&snapshot),
                       sizeof(snapshot));
}

bool relaySnapshotRecordEmpty(const GracefulRelaySnapshot &snapshot) {
  const uint32_t *words = reinterpret_cast<const uint32_t *>(&snapshot);
...
bool eraseLastRelaySnapshotSector() {
  return ESP.flashEraseSector(lastRelaySnapshotFlashBase() / kFlashSectorSize);
}

bool writeLastRelaySnapshotSlot(uint8_t slot, const GracefulRelaySnapshot &snapshot) {
  GracefulRelaySnapshot existing{};
  if (!readLastRelaySnapshotSlot(slot, existing) || !relaySnapshotRecordEmpty(existing)) return false;
  const uint32_t address = lastRelaySnapshotSlotAddress(slot);
  GracefulRelaySnapshot writable = snapshot;
  if (!ESP.flashWrite(address,
                      reinterpret_cast<uint32_t *>(&writable),
                      sizeof(writable))) {
    return false;
  }
  GracefulRelaySnapshot verify{};
  return readLastRelaySnapshotSlot(slot, verify) &&
         memcmp(&verify, &snapshot, sizeof(snapshot)) == 0 &&
         gracefulRelaySnapshotValid(verify);
}

bool clearLastRelaySnapshot() {
  last_relay_restore_valid = false;
  last_relay_restore_mask = 0;
  return eraseLastRelaySnapshotSector();
}
...
void loadLastRelaySnapshot() {
  ...
  const uint8_t count = lastRelaySnapshotRecordCount();
  for (uint8_t slot = 0; slot < count; slot++) {
    GracefulRelaySnapshot snapshot{};
    if (!readLastRelaySnapshotSlot(slot, snapshot)) continue;
    if (relaySnapshotRecordEmpty(snapshot)) break;
    if (gracefulRelaySnapshotValid(snapshot)) {
      found = true;
      found_mask = snapshot.relay_mask;
    }
  }

  if (found) {
    last_relay_restore_valid = true;
    last_relay_restore_mask = found_mask;
  }
}

bool saveLastRelaySnapshot() {
  if (runtime_template.relay_count == 0) return clearLastRelaySnapshot();
  ...
  int16_t empty_slot = -1;
  const uint8_t count = lastRelaySnapshotRecordCount();
  for (uint8_t slot = 0; slot < count; slot++) {
    GracefulRelaySnapshot existing{};
    if (!readLastRelaySnapshotSlot(slot, existing)) continue;
    if (relaySnapshotRecordEmpty(existing)) {
      empty_slot = slot;
      break;
    }
  }
  ...
  if (empty_slot < 0) {
    if (!eraseLastRelaySnapshotSector()) return false;
    empty_slot = 0;
  }
  if (!writeLastRelaySnapshotSlot(static_cast<uint8_t>(empty_slot), snapshot)) return false;
...
bool factoryResetConfig() {
  ...
  const bool relay_snapshot_erased = clearLastRelaySnapshot();
  setDefaultConfig();
  clearBootRecoveryLogShadow();
  return energy_erased && relay_snapshot_erased && commitConfig(true);
}
...
void clearTemplateConfig(StoredConfig &target) {
  ...
  setDefaultRelayPulseConfig(target);
}
...
void normalizeConfigStrings() {
  ...
  for (uint8_t i = 0; i < kMaxRelays; i++) {
    config.relay_restore_boot[i] = config.relay_restore_boot[i] ? 1 : 0;
    ...
    config.relay_pulse_enabled[i] = config.relay_pulse_enabled[i] ? 1 : 0;
    if (config.relay_pulse_enabled[i] &&
        (config.relay_pulse_seconds[i] < kRelayPulseMinSeconds ||
         config.relay_pulse_seconds[i] > kRelayPulseMaxSeconds)) {
...
void prepareConfigForStorage(StoredConfig &target) {
  ...
  target.crc = configCrc(target);
}

bool storedConfigMatchesConfig(const StoredConfig &target) {
  const uint8_t *stored = EEPROM.getConstDataPtr();
  if (!stored) return false;
  return memcmp(stored, &target, sizeof(target)) == 0;
}
...
void applyConfigMigrationDefaults(uint16_t previous_version) {
  ...
  if (previous_version < kConfigVersionV22) {
...
bool commitConfig(bool force_commit) {
  ...
  config.crc = configCrc(config);
  StoredConfig stored_config = config;
  prepareConfigForStorage(stored_config);
  if (!force_commit && storedConfigMatchesConfig(stored_config)) {
...
bool loadConfig() {
  ...
  if (header.version == kConfigVersionV23 && header.size == sizeof(StoredConfigV23)) {
    StoredConfigV23 *old_config = new StoredConfigV23;
    if (!old_config) {
      setDefaultConfig();
      return false;
    }
    ...
    memset(&config, 0, sizeof(config));
    memcpy(&config, old_config, offsetof(StoredConfigV23, crc));
    delete old_config;
    commitConfig();
    return config_ok;
  }

  if (header.version == kConfigVersionV22 && header.size == sizeof(StoredConfigV22)) {
    StoredConfigV22 *old_config = new StoredConfigV22;
    if (!old_config) {
      setDefaultConfig();
      return false;
    }
    ...
    memset(&config, 0, sizeof(config));
    memcpy(&config, old_config, offsetof(StoredConfigV22, crc));
    delete old_config;
    commitConfig();
    return config_ok;
  }

  if (header.version == kConfigVersionV21 && header.size == sizeof(StoredConfigV21)) {
    StoredConfigV21 *old_config = new StoredConfigV21;
    if (!old_config) {
      setDefaultConfig();
      return false;
    }
    ...
    memset(&config, 0, sizeof(config));
    memcpy(&config, old_config, offsetof(StoredConfigV21, crc));
    delete old_config;
    commitConfig();
    return config_ok;
  }
  ...
  if (header.version == kConfigVersionV16 && header.size == sizeof(StoredConfigV16)) {
    ...
    memset(&config, 0, sizeof(config));
    memcpy(&config, old_config, offsetof(StoredConfigV16, crc));
    for (uint8_t i = 0; i < kMaxRelays; i++) {
...
bool saveRelayPulseConfig(const uint8_t *pulse_enabled, const uint16_t *pulse_seconds) {
  memcpy(config.relay_pulse_enabled, pulse_enabled, sizeof(config.relay_pulse_enabled));
  memcpy(config.relay_pulse_seconds, pulse_seconds, sizeof(config.relay_pulse_seconds));
  if (!commitConfig()) return false;
...
void importSettingsSystem(JsonObjectConst root, StoredConfig &target, SettingsImportStats &stats) {
  ...
  if (!power_value.isNull()) {
    ...
    if (power_object.isNull()) {
      ...
      JsonVariantConst mode_value = power_object["mode"];
      ...
      JsonVariantConst persist_value = power_object["persist"];
      if (!persist_value.isNull()) {
        bool enabled = false;
      ...
      JsonVariantConst locked_value = power_object["locked"];
      if (!locked_value.isNull()) {
        bool enabled = false;
  ...
  JsonVariantConst persist_value = system["power_saving_persist"];
  if (!persist_value.isNull()) {
    bool enabled = false;
    if (settingsReadBool(persist_value, enabled)) {
      target.power_saving_persist = enabled ? 1 : 0;
  ...
  JsonVariantConst locked_value = system["power_saving_locked"];
  if (!locked_value.isNull()) {
    bool enabled = false;
    if (settingsReadBool(locked_value, enabled)) {
      target.power_saving_locked = enabled ? 1 : 0;
...
void importSettingsRelayEnforcement(JsonObjectConst root, StoredConfig &target, const RuntimeTemplate &rt, SettingsImportStats &stats) {
  ...
  for (uint8_t i = 0; i < count; i++) {
    ...
    if (relay.containsKey("restore_boot")) {
      JsonVariantConst restore_boot = relay["restore_boot"];
      if (restore_boot.is<bool>()) {
        target.relay_restore_boot[i] = restore_boot.as<bool>() ? 1 : 0;
...
void importSettingsRelayPulsing(JsonObjectConst root, StoredConfig &target, const RuntimeTemplate &rt, SettingsImportStats &stats) {
  JsonVariantConst relays_value = root["relay_pulsing"];
  if (relays_value.isNull()) return;
  JsonArrayConst relays = relays_value.as<JsonArrayConst>();
  if (relays.isNull()) {
  ...
  for (uint8_t i = 0; i < count; i++) {
    JsonObjectConst relay = relays[i].as<JsonObjectConst>();
    if (relay.isNull()) {
      if (!relays[i].isNull()) recordSettingsSkipped(stats, String(F("relay_pulsing[")) + String(i) + F("]"));
      continue;
    }
    if (!relayAvailableIn(rt, i)) continue;
    ...
    if (relay.containsKey("enabled")) {
      if (!settingsReadBool(relay["enabled"], enabled)) {
    ...
    uint16_t seconds = 0;
    if (settingsReadUint16(relay["seconds"], kRelayPulseMinSeconds, kRelayPulseMaxSeconds, seconds)) {
...
void applyApiPowerSavingSetting(JsonVariantConst value, StoredConfig &target, SettingsImportStats &stats,
  ...
  if (object.isNull()) {
    uint8_t mode = kPowerSavingOff;
    if (settingsReadPowerSavingMode(value, mode)) {
      target.power_saving_mode = mode;
      recordSettingsApplied(stats);
    } else {
  ...
  JsonVariantConst mode_value = object["mode"];
  if (!mode_value.isNull()) {
    uint8_t mode = kPowerSavingOff;
    if (settingsReadPowerSavingMode(mode_value, mode)) {
      target.power_saving_mode = mode;
      ...
      recordSettingsSkipped(stats, field + F(".mode"));
    }
  }
  JsonVariantConst persist_value = object["persist"];
  if (!persist_value.isNull()) {
    bool enabled = false;
    if (settingsReadBool(persist_value, enabled)) {
      target.power_saving_persist = enabled ? 1 : 0;
      ...
      recordSettingsSkipped(stats, field + F(".persist"));
    }
  }
  JsonVariantConst locked_value = object["locked"];
  if (!locked_value.isNull()) {
    bool enabled = false;
    if (settingsReadBool(locked_value, enabled)) {
      target.power_saving_locked = enabled ? 1 : 0;
      ...
      recordSettingsSkipped(stats, field + F(".locked"));
    }
  }
}

void applyApiPowerSavingPersistSetting(JsonVariantConst value, StoredConfig &target, SettingsImportStats &stats,
                                       const String &field) {
  bool enabled = false;
...
void applyApiPowerSavingLockedSetting(JsonVariantConst value, StoredConfig &target, SettingsImportStats &stats,
                                      const String &field) {
  bool enabled = false;
  if (settingsReadBool(value, enabled)) {
    target.power_saving_locked = enabled ? 1 : 0;
...
void applyApiSystemSettings(JsonObjectConst root, StoredConfig &target, SettingsImportStats &stats) {
  ...
  if (root.containsKey("power_saving_persist")) {
    applyApiPowerSavingPersistSetting(root["power_saving_persist"], target, stats, F("power_saving_persist"));
  }
  if (root.containsKey("power_saving_locked")) {
    applyApiPowerSavingLockedSetting(root["power_saving_locked"], target, stats, F("power_saving_locked"));
  }
  ...
  if (!system.isNull()) {
    ...
    if (system.containsKey("power_saving_persist")) {
      applyApiPowerSavingPersistSetting(system["power_saving_persist"], target, stats, F("system.power_saving_persist"));
    }
    if (system.containsKey("power_saving_locked")) {
      applyApiPowerSavingLockedSetting(system["power_saving_locked"], target, stats, F("system.power_saving_locked"));
    }
...
bool apiSettingsGetHasUpdateArgs() {
  if (server.hasArg(F("power_saving")) ||
      server.hasArg(F("power_saving_mode")) ||
      server.hasArg(F("power_saving_persist")) ||
      server.hasArg(F("power_saving_locked"))) return true;
...
bool applyApiSettingsGetArgs(StoredConfig &target, SettingsImportStats &stats) {
  ...
  if (apiSettingsGetArg(F("power_saving_persist"), F(""), power_saving_persist)) {
    ...
    if (parseBoolText(power_saving_persist, enabled)) {
      ...
      recordSettingsSkipped(stats, F("query.power_saving_persist"));
    }
  }
  ...
  if (apiSettingsGetArg(F("power_saving_locked"), F(""), power_saving_locked)) {
    ...
    if (parseBoolText(power_saving_locked, enabled)) {
      ...
      recordSettingsSkipped(stats, F("query.power_saving_locked"));
    }
  }
...
"</style></head><body>");
  ...
  page += F("</span><span>my<b>Mota</b></span></a><div class='host'><span class='dot'></span>");
  page += host;
...
void appendFooter(String &page, bool live_poll = true, bool reboot_wait = false) {
  page += F("<div class='foot'>myMota &middot; ESP8266/ESP8285 firmware &middot; ");
  page += F(MYMOTA_VERSION);
  page += F("</div>");
...
void appendDeviceControls(String &page) {
  ...
  if (energy.present) {
    ...
    page += F("'></div></div><button type='submit'>Save energy</button></form></div>");
  }
  page += F("</section>");
}
...
void appendButtonSettings(String &page) {
  ...
  for (uint8_t i = 0; i < runtime_template.button_count; i++) {
    ...
    page += F(">Yes, GPIO low is ON</option></select></div></div>");
...
void handleSettingsImport() {
  ...
  importSettingsRelayPulsing(root, candidate, candidate_runtime, stats);
...
void handleHealth() {
  ...
  if (last_wifi_begin_attempt == 0) {
    out += F("null");
  } else {
    out += millis() - last_wifi_begin_attempt;
  }
  out += F(",\"wifi_status_false_ms\":");
  if (wifi_sdk_connected || !disconnected_timer_active) {
    out += F("0");
  } else {
    out += millis() - disconnected_since;
  }
...
void setupRoutes() {
  ...
  server.on(F("/relay-pulsing"), HTTP_POST, handleRelayPulseSave);
...
void setup() {
  ...
  if (!serialClaimed()) {
    Serial.printf("HTTP server started; STA %s AP %s\n",
                  wifiUsable() ? WiFi.localIP().toString().c_str() : "not-connected",
